...
def bbox_coco(list_of_value):
    list_of_point = list_of_value
    min_point = [min([p[0] for p in list_of_point]), min([p[1] for p in list_of_point])]
    max_point = [max([p[0] for p in list_of_point]), max([p[1] for p in list_of_point])]
    width = round(max_point[0] - min_point[0], 2)
    height = round(max_point[1] - min_point[1], 2)
    result = [min_point[0], min_point[1], width, height]
    return result
...
def area_calculator(list_of_value):
    ...
    final_sum = abs((sum_1 - sum_2) / 2.0)
    return round(final_sum, 2)

def creation_json_yolo_to_coco(images_directory, annotation_directory, categories,anno_file_name):
    codified_categories = creates_categories(categories)
    base_json = {
    ...
    for file in os.listdir(images_directory):
        image = {
            "id": image_id, "width": "", "height": "", "file_name": "", "license": 0,
            "flickr_url": "", "coco_url": "", "date_captured": 0
        }
        im = Image.open(os.path.join(images_directory, file))
        w, h = im.size
        ...
        base_json["images"].append(image)
        annotation_file = file[:-3] + "txt"
        with open(os.path.join(annotation_directory, annotation_file), "r") as new_file:
            for line in new_file:
                annotation = {
                    "id": annotation_id, "image_id": image_id, "category_id": "", "segmentation": [],
                    "area": "", "bbox": [], "iscrowd": 0, "attributes": {"occluded": False}
                }
                ann = line.split()
                point_in_coco_format = yolo_coord_to_coco(list_of_value=ann, width=w, height=h)
    ...
    with open(f"{anno_file_name}.json", "w") as outfile:
